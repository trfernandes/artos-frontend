...
Padrão oficial: **flat** — sem `AuthLayout`, sem gradiente, sem card manual. Segue o padrão de login/recovery/cadastro (telas existentes).

Conteúdo do form de toda tela de auth deve ser **centralizado verticalmente** (logo do login é exceção).
...
Wrappers disponíveis em `components/forms/`: ControlledTextInput, ControlledPasswordInput, ControlledDropDown, ControlledBottomSheetSelect, ControlledTextArea, ControlledNumberInput, ControlledMaskedTextInput, ControlledImagePicker, ControlledSearchSelect, ControlledSelectField, ControlledFancyToggle, ControlledDateInput, ControlledColorPicker.
...
- Após implementar: self-review — o resultado bate com o spec? (grid está em 2 colunas? badge aparece?)
...
- Padrão: `<View style={{ alignItems: 'center', paddingVertical: 32 }}><ActivityIndicator size='large' /></View>`
...
- `FancyAlert` com 3+ botões: primeiro sem style = `contained`, `style:'default'` = `outlined`, `style:'cancel'` = `text`

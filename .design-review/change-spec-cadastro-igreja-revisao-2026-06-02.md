...
**App**: Artos (Diakonia) **Intent**: consistência geral **States reviewed**: Atual (single-state)
...
- **Element**: faixa de trial, texto "· a partir de R$ 39,90/mês" em
  `create-igreja-account-tab-pronto.tsx`
- **From**: `color={palette.fonts.inactive}` (#6F6F6F sobre lavanda primary@0.10) → 4.19:1,
  reprovava WCAG AA
...
- **Element**: ações "Voltar" do wizard em `app/(auth)/create-igreja-account.tsx` (passos Contato e
  Revisão)

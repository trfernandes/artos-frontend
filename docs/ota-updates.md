...
`eas update` **não lê** `eas.json > build.<profile>.env`. Ele roda o Metro localmente e carrega
`.env` + `.env.local` (nessa ordem, `.env.local` sempre vence). Como esses dois apontam pro backend
de **staging**, um `eas update --channel production` cru publica o bundle de produção falando com o
banco de staging.
...
(`diakonia-backend-staging` sem `.onrender.com` aparece de propósito — é a string de comparação do
`utils/featureFlags.ts`.)

## Fix definitivo (pendente)

Migrar a env pro ambiente hospedado do EAS, aí `--environment production` puxa do servidor e não
...
Update só chega em builds com o mesmo `runtimeVersion` do bundle da loja (hoje `1.1.1`). Mudança de
dependência nativa quebra isso — aí é build novo + submit, não OTA.

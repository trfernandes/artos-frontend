...
navegando de verdade entre várias telas do app (ex: trilha de Escalas: lista → detalhes → gerar com
assistente → personalizar escala gerada).

Ao planejar a arquitetura técnica dessa feature, esse modelo foi revisto: tours forçados no primeiro
login e que empurram o usuário por múltiplas telas tendem a ter alta taxa de dispensa — padrão
observado em produtos SaaS (ex: Figma usa overlays contextuais por feature, não um tour upfront
encadeado). Fonte:
...
- Cada tour é confinado a uma única tela — não há orquestração de navegação entre telas durante o
  tour.
- O gatilho é um banner dispensável, mostrado apenas na primeira visita de cada tela ("Quer conhecer
  essa tela? Iniciar Tutorial / Agora não").
...
- **Positivo**: escopo técnico mais simples (sem orquestração de navegação cross-screen), alinhado a
  práticas de mercado, menor risco de o usuário abandonar o tutorial por sentir-se forçado.

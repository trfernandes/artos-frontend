...
export function useEventoSetlistObservacoes(eventoId?: string, dataOcorrencia?: string, ministerioId?: string) {
  ...
  const queryKey = ['evento-setlist-observacoes', igrejaAtiva.id, eventoId, ministerioId, dataOcorrencia];

  const query = useQuery({
    queryKey,
    enabled: !!eventoId && !!ministerioId && !!dataOcorrencia,
    queryFn: () => IgrejaEventosRepository.obterObservacoesSetlist(igrejaAtiva.id, eventoId!, ministerioId!, dataOcorrencia!),
  });

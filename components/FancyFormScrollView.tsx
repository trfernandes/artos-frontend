...
/**
 * Scroll para formulários longos, baseado em `react-native-keyboard-controller`
 * (mesmo motor de teclado do AuthLayout). Substitui o FancyScrollView — que usa a
 * lib antiga `react-native-keyboard-aware-scroll-view` — em telas onde alternar
 * o foco entre campos com o teclado aberto fechava o teclado.
 *
 * Mantém os fades de topo/base do FancyScrollView; troca apenas o motor de scroll.
 * Use em forms longos (ex.: Novo evento). Para listas/scroll comuns sem teclado,
 * continue usando FancyScrollView.
 */
export type FancyFormScrollViewProps = ScrollViewProps & {
  children?: React.ReactNode;
  /** Ocupa o espaço do pai e rola dentro (flex:1). */
  fill?: boolean;
  containerStyle?: StyleProp<ViewStyle>;
  /** Distância mínima entre o campo focado e o topo do teclado. Default 20. */
  bottomOffset?: number;
  /** Compensação extra de espaço quando o teclado abre (ex.: footer fixo). */
...
export default function FancyFormScrollView(props: FancyFormScrollViewProps) {
  // Android: faz a janela redimensionar quando o teclado abre (adjustResize).
  useResizeMode();

  const { fill = false, bottomOffset = 20, extraKeyboardSpace } = props;

...
export function TutorialSpotlight({ rect, padding = 6 }: TutorialSpotlightProps) {
  ...
  return (
    <View style={StyleSheet.absoluteFill} pointerEvents="box-only">
      <View style={[styles.mask, { backgroundColor: overlayColor, top: 0, left: 0, right: 0, height: spot.y }]} />
      <View
        style={[
          styles.mask,
          { backgroundColor: overlayColor, top: spot.y + spot.height, left: 0, right: 0, bottom: 0 },
        ]}
      />
      <View
        style={[
          styles.mask,
          { backgroundColor: overlayColor, top: spot.y, height: spot.height, left: 0, width: spot.x },
        ]}
      />
      <View
        style={[
          styles.mask,
          { backgroundColor: overlayColor, top: spot.y, height: spot.height, left: spot.x + spot.width, right: 0 },
        ]}
      />
      <View
        pointerEvents="none"
        style={[
          ...
          { borderColor: Pallete.primary, top: spot.y, left: spot.x, width: spot.width, height: spot.height },
        ]}
      />
    </View>
  );

...
export default function JoinChurchModal(props: FancyModalDialogProps<any>) {
  ...
  // Extrair token de URLs como diakonia://invite/TOKEN, https://diakonia.app.br/invite/TOKEN ou apenas TOKEN
  const extractToken = (input: string): string => {
    const trimmed = input.trim();

    // Tentar extrair de deeplink (diakonia://invite/TOKEN)
    const deeplinkMatch = trimmed.match(/diakonia:\/\/invite\/([^\/\s]+)/);
    if (deeplinkMatch) {
      return deeplinkMatch[1];
    }

    // Tentar extrair de URL HTTPS (https://diakonia.app.br/invite/TOKEN)
    const httpsMatch = trimmed.match(/https?:\/\/diakonia\.app\.br\/invite\/([^\/\s]+)/);
    if (httpsMatch) {
      return httpsMatch[1];
    }
  ...
  const handlePreview = async () => {
    ...
    if (!extractedToken) {
      Toast.show({
        type: 'error',
        text1: 'Digite o código da igreja',
      });
      return;
    }
    ...
    >
      ...
      {!preview ? (
        <>
          ...
          <FancyButton
            label={loadingPreview ? 'Verificando...' : 'Verificar código'}

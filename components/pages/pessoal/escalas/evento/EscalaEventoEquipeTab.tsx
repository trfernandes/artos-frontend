...
export default function EscalaEventoEquipeTab({
  ...
  const voluntariosData = useMemo<DataType<'image'>[]>(() => {
    ...
    return data.map(item => {
      const imageSource =
        ImageUtils.rawToDataUri(item.voluntario?.voluntario?.foto) ??
        item.voluntario?.voluntario?.foto ??
        require('../../../../../assets/images/empty_profile_image.png');

      return {
        key: item.voluntario?.voluntario?.id,
        title: user?.id === item.voluntario?.voluntario?.id ? 'Você' : item.voluntario?.voluntario?.nome,
        subtitle: item.funcao?.nome,
        highlighted: user?.id === item.voluntario?.voluntario?.id,
        source: imageSource || require('../../../../../assets/images/empty_profile_image.png'),
        linkedData: item,
      } as DataType<'image'>;
    });
  }, [data]);

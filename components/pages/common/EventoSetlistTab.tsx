...
export default function EventoSetlistTab({ eventoId, dataOcorrencia, ministerioId, canEdit }: Props) {
  ...
  const { data, isLoading, criarSetlistItem, atualizarSetlistItem, removerSetlistItem, reordenarSetlist, isMutatingSetlist } =
    useEventoSetlist(eventoId, dataOcorrenciaIso);
  ...
  const moveItem = async (itemId: string, direction: -1 | 1) => {
    ...
    await reordenarSetlist({ dataOcorrencia: dataOcorrenciaIso, itemIds: reordered });
  };

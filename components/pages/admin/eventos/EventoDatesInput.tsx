...
export default function EventoDatesInput({ disabled = false }: { disabled?: boolean }) {
  ...
  return (
    <View style={{ gap: 15 }}>
      <View style={{ gap: 5 }}>
        <FancySettingItem
          icon={{ library: 'MaterialCommunityIcons', name: 'calendar-clock', size: 20 }}
          label="Começa"
          options={[]}
          ...
          <View style={{ flexDirection: 'row', gap: 5 }}>
            ...
            <FancyTimePickerModal
              ...
              onChange={time => {
                ...
                const newDate = new Date(Number(year), Number(month) - 1, Number(day), time.hour, time.minute);
                console.log('newDate', format(newDate, 'dd/MM/yyyy HH:mm'));
                onChange(newDate);
              }}
              containerStyle={{ width: 100 }}
            />
